...
public:
    DummyAction(const std::string& name = "Enquiring", int type = 0) : name_(name), type_(type) {}
    std::string getActionName() const override { return name_; }
    int getQuestionType() const override { return type_; }
    int getEnquiryQuestionType() const override { return type_; }
private:
    std::string name_;
    int type_;
};
...
public:
    DummyCustomer2(const std::string& id = "C1", Action* action = nullptr) : Customer(nullptr, nullptr), id_(id), action_(action) {}
    std::string getId() const override { return id_; }
    Action* getAction() const override { return action_; }
    void setAssignedStaff(Staff* s) override { assigned_ = s; }
    Staff* getAssignedStaff() const override { return assigned_; }
private:
    std::string id_;
    Action* action_;
    Staff* assigned_ = nullptr;
};
...
TEST(InfoDeskTest, HandleCustomerNotEnquiring) {
    InfoDesk desk;
    DummyAction notEnquiring("NotEnquiring", 0);
...
public:
    ...
    std::string getStaffType() const override { return "SalesStaff"; }
...
public:
    ...
    std::string getId() const override { return "C1"; }
    Action* getAction() const override { return nullptr; }
};
...
TEST(StaffTest, SetAndGetRole) {
    ...
    EXPECT_EQ(staff.getRole(), "TestRole");
}
...
TEST(ManagerTest, RoleIsManager) {
    ...
    manager.setRole();
    EXPECT_EQ(manager.getRole(), "Manager");
}
...
TEST(SalesStaffTest, RoleIsSalesStaff) {
    SalesStaff sales;
    sales.setRole();
    EXPECT_EQ(sales.getRole(), "SalesStaff");
}
